<div class="container mx-auto px-2 md:px-10 lg:px-14 py-6 md:py-10">
  <h1 class="text-xl md:text-3xl lg:text-4xl font-bold font-telkomsel text-gray-800 mb-4 md:mb-6 px-2">
    Pilihan Paket Produk Internet Yang Kami Sediakan
  </h1>

  <div class="bg-gradient-to-r from-[#001a41] to-[#0e336c] p-4 md:p-6 rounded-2xl shadow-lg mx-2">
    <div class="flex flex-col space-y-4 md:space-y-0 md:flex-row justify-between items-start md:items-center">
      <div class="w-full">
        <h2 class="text-lg md:text-2xl lg:text-3xl font-bold font-telkomsel text-white mb-2 md:mb-4">
          Cari Berdasarkan Kategori
        </h2>
        <p class="text-sm md:text-base text-white mb-4">
          Temukan Pilihan Paket Internet Dengan Kebutuhan Yang Anda Inginkan
        </p>

        <div class="flex items-center space-x-2">
          <!-- Tombol Filter Mobile -->
          <div class="block md:hidden">
            @include('produk.filter-mobile')
          </div>
          <div class="relative w-full md:w-1/3 lg:w-1/4 md:ml-4 font-telkomsel">
            <select id="kategori-filter-mobile" name="kategori"
...
<script>
  ...
  function filterProduk() {
    ...
    let kategori = document.getElementById('kategori-filter-mobile').value;  // Get selected category
    ...
    $.ajax({
      ...
      success: function (response) {
        $('#produk-container-mobile').html(response);  // Update products in the page
      },
  ...
  document.getElementById('kategori-filter-mobile').addEventListener('change', filterProduk);  // Filter when category changes
