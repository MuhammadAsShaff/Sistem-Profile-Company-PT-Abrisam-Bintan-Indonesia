<form method="GET" action="{{ route('dashboard.dataPelanggan.dataPelanggan') }}" class="max-w-lg mx-auto">
  <div class="flex">

    <!-- Tombol Dropdown untuk Kategori Status Customer -->
    <button id="dropdown-button" data-dropdown-toggle="dropdown"
      ...
      type="button">
      {{ request('status') ? request('status') : 'Semua Status' }}
    ...
    <!-- Daftar Dropdown untuk Status Customer -->
    <div id="dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
      <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
        <li>
          ...
          @foreach($statusCustomer as $stat)
      <li>
        <button type="submit" name="status" value="{{ $stat }}"
        class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
        {{ $stat }}
        </button>
      </li>
