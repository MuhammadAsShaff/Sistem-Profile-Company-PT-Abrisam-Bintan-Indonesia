...
<button type="button" onclick="openModal('popup-modal-{{ $item->id_inventoryKeluar }}')"
...
<!-- Modal -->
<dialog id="popup-modal-{{ $item->id_inventoryKeluar }}" class="modal rounded-lg shadow-lg modal-hide"
  style="transform: translate(-50%, -50%);">
  <div class="relative bg-white rounded-xl shadow dark:bg-gray-700 p-4 md:p-5 text-center">

    <!-- Close button -->
    <button type="button" onclick="closeModal('popup-modal-{{ $item->id_inventoryKeluar }}')"
    ...
    <!-- Form to delete the inventory -->
    <form action="{{ route('inventoryKeluar.delete', $item->id_inventoryKeluar) }}" method="POST" class="inline">
      @csrf
      @method('DELETE')
      <button type="submit"
