<div class="flex items-center justify-between mt-6">
  <!-- Previous Button -->
  @if ($inventoryKeluar->onFirstPage())
    <span
  ...
  @else
    <a href="{{ $inventoryKeluar->previousPageUrl() }}"
  ...
  <div class="items-center hidden lg:flex gap-x-3">
    @foreach ($inventoryKeluar->getUrlRange(1, $inventoryKeluar->lastPage()) as $page => $url)
    @if ($page == $inventoryKeluar->currentPage())
...
@endif
  ...
  <!-- Next Button -->
  @if ($inventoryKeluar->hasMorePages())
    <a href="{{ $inventoryKeluar->nextPageUrl() }}"
    class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
    <span>Next</span>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
      class="w-5 h-5 rtl:-scale-x-100">
