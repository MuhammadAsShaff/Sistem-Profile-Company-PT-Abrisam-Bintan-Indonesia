...
<button
  ...
  onclick="openModal('produkModal-{{ $kategori->id_kategori }}')">
  Lihat Produk
</button>

<!-- Modal Produk -->
<dialog id="produkModal-{{ $kategori->id_kategori }}"
  ...
  <div class="relative bg-white rounded-lg shadow-lg p-6 h-full" id="modalContent" style="overflow-y: auto;">
    <div class="flex items-start justify-between p-5 border-b border-gray-200 rounded-t mb-6">
      <h3 class="text-lg font-semibold text-gray-900">Daftar Produk di Kategori {{ $kategori->nama_kategori }}</h3>
      <button type="button"
        class="text-gray-400 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex items-center"
        onclick="closeModal('produkModal-{{ $kategori->id_kategori }}')">
    ...
    <div class="modal-body" style="max-height: 60vh; overflow-y: auto;">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @if (isset($kategori->produk) && count($kategori->produk) > 0)
